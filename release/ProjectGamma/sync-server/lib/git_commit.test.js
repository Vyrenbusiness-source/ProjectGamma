...
test("commitChanges: selektives staging via filesChanged — nur gelistete files", async () => {
  const dir = tempRepo();
  fs.writeFileSync(path.join(dir, "target.txt"), "target\n");
  fs.writeFileSync(path.join(dir, "bystander.txt"), "bystander\n");
  const r = await commitChanges({
    projectPath: dir, message: "[cc] selective",
    filesChanged: ["target.txt"],
  });
  assert.equal(r.committed, true);
  assert.deepEqual(r.stagedFiles, ["target.txt"]);
  // bystander.txt darf nicht im commit sein
  const show = spawnSync("git", ["show", "--name-only", "--format=", "HEAD"], { cwd: dir, shell: true, encoding: "utf8" });
  assert.ok(show.stdout.includes("target.txt"), "target.txt soll committed sein");
  assert.ok(!show.stdout.includes("bystander.txt"), "bystander.txt darf NICHT committed sein");
  fs.rmSync(dir, { recursive: true, force: true });
});

test("commitChanges: filesChanged blockiert path-traversal (../)", async () => {
  const dir = tempRepo();
  fs.writeFileSync(path.join(dir, "ok.txt"), "ok\n");
  const r = await commitChanges({
    projectPath: dir, message: "[cc] traversal-test",
    filesChanged: ["ok.txt", "../etc/passwd", "ok.txt\0evil"],
  });
  assert.equal(r.committed, true);
  assert.deepEqual(r.stagedFiles, ["ok.txt"]);
  fs.rmSync(dir, { recursive: true, force: true });
});
